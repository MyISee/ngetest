...
async function starts() {
	...
	ya.logger.level = 'warn'
        ya.version = [2, 2123, 8]
	console.log('>', '[',color('INFO','blue'),']','Starting Yan Bot...')
  console.log('>', '[',color('INFO','blue'),']','Configure Connection...')
  console.log('>', '[',color('INFO','blue'),']','Configure Success, Connecting')
	ya.on('qr', () => {
	console.log(color('[','white'), color('!','red'), color(']','white'), color(' SCAN ME'))
	})

	fs.existsSync('./iyan.json') && ya.loadAuthInfo('./iyan.json')
	ya.on('connecting', () => {
	start('2', 'Connecting...')
	})
	ya.on('open', () => {
...
ya.on("group-update", async (anu) => {
    ...
    if (anu.announce == "false") {
      ...
      ya.sendMessage(metdata.id, teks, MessageType.text);
...
ya.on("group-participants-update", async (anu) => {
    try {
      groupMet = await ya.groupMetadata(anu.jid);
      ...
      console.log(anu);
      try {
        pp_user = await ya.getProfilePicture(mem);
      } catch (e) {
        pp_user =
          "https://ibb.co/bBXCV8h";
      }
      try {
        pp_grup = await ya.getProfilePicture(anu.jid);
      } catch (e) {
        pp_grup =
          "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png?q=60";
      }
      if (anu.action == "add" && !mem.includes(ya.user.jid)) {
        mdata = await ya.groupMetadata(anu.jid);
        memeg = mdata.participants.length;
        num = anu.participants[0];
        let v = ya.contacts[num] || { notify: num.replace(/@.+/, "") };
        ...
        buttonsMessage = {
          contentText: `${teks}`,
          footerText: "jual sc no enc 15k",
          imageMessage: imageMsg,
          buttons: buttons,
          headerType: 4,
        };
        prep = await ya.prepareMessageFromContent(
          mdata.id,
          { buttonsMessage },
          {}
        );
      ...
      if (anu.action == "remove" && !mem.includes(ya.user.jid)) {
        mdata = await ya.groupMetadata(anu.jid);
        num = anu.participants[0];
        let w = ya.contacts[num] || { notify: num.replace(/@.+/, "") };
        anu_user = w.vname || w.notify || num.split("@")[0];
        time_wel = moment.tz("Asia/Jakarta").format("HH:mm");
        memeg = mdata.participants.length;
        out = `Yahhh ada yang keluar\n${anu_user} kalo balik beliin ayam goyeng`;
        buff = await getBuffer(
...
ya.on('CB:action,,call', async json => {
...
ya.on('chat-update', async (iyan) => {
		try {
			if (!iyan.hasNewMessage) return
            iyan = iyan.messages.all()[0]
			if (!iyan.message) return
			if (iyan.key && iyan.key.remoteJid == 'status@broadcast') return
			if (iyan.key.fromMe) return			
            m = simple.smsg(ya, iyan)
            global.ky_ttt
            global.join_fer
            global.prefix
			global.blocked
            iyan.message = (Object.keys(iyan.message)[0] === 'ephemeralMessage') ? iyan.message.ephemeralMessage.message : iyan.message
            const content = JSON.stringify(iyan.message)
			const from = iyan.key.remoteJid
            ...
            const cmod = (type === 'conversation' && iyan.message.conversation) ? iyan.message.conversation : (type == 'imageMessage') && iyan.message.imageMessage.caption ? iyan.message.imageMessage.caption : (type == 'videoMessage') && iyan.message.videoMessage.caption ? iyan.message.videoMessage.caption : (type == 'extendedTextMessage') && iyan.message.extendedTextMessage.text ? iyan.message.extendedTextMessage.text : ''.slice(1).trim().split(/ +/).shift().toLowerCase()
		    const prefix = /^[°•π÷×¶∆£¢€¥®™✓=|~zZ+×_*!#$%^&./\\©^]/.test(cmod) ? cmod.match(/^[°•π÷×¶∆£¢€¥®™✓=|~zZ+×_*!#$,|`÷?;:%abcdefghijklmnopqrstuvwxyz%^&./\\©^]/gi) : '/'
            body = (type === 'conversation' && iyan.message.conversation.startsWith(prefix)) ? iyan.message.conversation : (type == 'imageMessage') && iyan.message[type].caption.startsWith(prefix) ? iyan.message[type].caption : (type == 'videoMessage') && iyan.message[type].caption.startsWith(prefix) ? iyan.message[type].caption : (type == 'extendedTextMessage') && iyan.message[type].text.startsWith(prefix) ? iyan.message[type].text : (type == 'listResponseMessage') && iyan.message[type].singleSelectReply.selectedRowId ? iyan.message[type].singleSelectReply.selectedRowId : (type == 'buttonsResponseMessage') && iyan.message[type].selectedButtonId ? iyan.message[type].selectedButtonId : (type == 'stickerMessage') && (getCmd(iyan.message[type].fileSha256.toString('base64')) !== null && getCmd(iyan.message[type].fileSha256.toString('base64')) !== undefined) ? getCmd(iyan.message[type].fileSha256.toString('base64')) : ""
			budy = (type === 'conversation') ? iyan.message.conversation : (type === 'extendedTextMessage') ? iyan.message.extendedTextMessage.text : ''
            ///button = (type == 'buttonsResponseMessage') ? iyan.message.buttonsResponseMessage.selectedDisplayText : ''
			...
			mess = {
				...
				only: {
					...
					benned: '*Maaf kamu di banned Owner*',
					owner: '*Command Ini Hanya Bisa Di Gunakan Oleh Owner!*',
					admin: 'Perintah ini hanya bisa di gunakan oleh admin group!',
					Badmin: 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin! ',					
				}
			}
		    const botNumber = ya.user.jid
			const ownerNumber = ["6289649627900@s.whatsapp.net"] // Nomor Owner Bre
			const ownerContact = ["6289649627900","6285725486855"] // Nomor Owner Bre
			const isGroup = from.endsWith('@g.us')
			const totalchat = await ya.chats.all()
			const sender = isGroup ? iyan.participant : iyan.key.remoteJid
			const groupMetadata = isGroup ? await ya.groupMetadata(from) : ''
			...
			const reply = (teks) => {
				ya.sendMessage(from, teks, text, {quoted:faketroli})
			}
			const testqq = (from, desk) => {
            ya.sendButtons(from, `Pilihan Anda Salah!!`, `Pilih Yang Bener, Jangan Ngasal!!`,`Pilih Enable atau Disable`, [{"buttonId": `${prefix}${desk} enable`,"buttonText": {"displayText": "ENABLE"},"type": "RESPONSE"},{"buttonId": `${prefix}${desk} disable`,"buttonText": {"displayText": "DISABLE"},"type": "RESPONSE"}], false, null)
            }
			const sendMess = (hehe, teks) => {
				ya.sendMessage(hehe, teks, text)
			}
			   const costum = (pesan, tipe, target, target2) => {
			ya.sendMessage(from, pesan, tipe, {quoted: { key: { fromMe: false, participant: `${target}`, ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target2}` }}})
			}
			const mentions = (teks, memberr, id) => {
				(id == null || id == undefined || id == false) ? ya.sendMessage(from, teks.trim(), extendedText, {contextInfo: {"mentionedJid": memberr}}) : ya.sendMessage(from, teks.trim(), extendedText, {quoted: faketroli, contextInfo: {"mentionedJid": memberr}})
			}
			
			if (vn) {
		    ya.updatePresence(from, Presence.recording)
		    } else if (ngetik) {
	        ya.updatePresence(from, Presence.composing)
		    }
            ya.chatRead(from, "read")
        // DATE WAKTU
        var ase = new Date();
                        var waktoo = ase.getHours();
                        switch(waktoo){
                case 0: waktoo = "Waktu Tengah Malam 🌚 - Tidur"; break;
                case 1: waktoo = "Waktu Tengah Malam 🌚 - Tidur sayang :)"; break;
                case 2: waktoo = "Waktu Dini Hari 🌒 - Tidur sayang :)"; break;
                case 3: waktoo = "Waktu Dini Hari 🌓 - Tidur sayang :)"; break;
                case 4: waktoo = "Subuh 🌔"; break;
                case 5: waktoo = "Subuh 🌔"; break;
                case 6: waktoo = "Selamat Pagi sayang 🌝🌝"; break;
                case 7: waktoo = "Selamat Pagi sayang 🌝🌝"; break;
                case 8: waktoo = "Selamat Pagi sayang 🌝🌝"; break;
                case 9: waktoo = "Selamat Pagi sayang 🌝🌝"; break;
                case 10: waktoo = "Selamat Pagi sayang 🌝"; break;
                case 11: waktoo = "Selamat Siang sayang 🌞\n - Jangan Lupa Shalat Dzuhur"; break;
                case 12: waktoo = "Selamat Siang sayang 🌞\n - Jangan Lupa Shalat Dzuhur"; break;
                case 13: waktoo = "Selamat Siang sayang 🌞\n - Jangan Lupa Shalat Dzuhur"; break;
                case 14: waktoo = "Selamat Siang sayang 🌞\n - Jangan Lupa Beristirahat :)"; break;
                case 15: waktoo = "Selamat Sore sayang 🌝\n -  Jangan Lupa Mandi Dan shalat ashar"; break;
                case 16: waktoo = "Selamat Sore sayang 🌝\n  - Jangan Lupa Mandi Dan shalat ashar"; break;
                case 17: waktoo = "Selamat Sore sayang 🌖\n - Menjelang Malam 🌚"; break;
                case 18: waktoo = "Waktu Magrib 🌘\n - Jangan Lupa Shalat Magrib sayang"; break;
                ...
                case 23: waktoo = "Tengah Malam 🌚 \n - Tidur sayang, Ga baik bergadang :)"; break;
            }
...
encmedia = content.includes("quotedMessage") ? JSON.parse(JSON.stringify(iyan).replace('quotedM','m')).message[obj].contextInfo : iyan
if (save) return await ya.downloadAndSaveMediaMessage(encmedia)
return await ya.downloadMediaMessage(encmedia)
  }
...
const sendStickerFromUrl = async(to, url) => {
                ...
                download(url, './tmp/stik' + names + '.png', async function () {
                    ...
                    exec(`ffmpeg -i ${filess} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${asw}`, (err) => {
                        ...
                        ya.sendMessage(to, media, MessageType.sticker,{quoted:faketroli})
...
const sendMediaURL = async(to, url, text="", mids=[]) =>{
                ...
                download(url, filename, async function () {
                    ...
                    ya.sendMessage(to, media, type, { quoted: faketroli, mimetype: mime, caption: text,contextInfo: {"mentionedJid": mids}})
...
const sendFileFromUrl = async(link, type, options) => {
  hasil = await getBuffer(link).catch(e => {
	...
	return ya.sendMessage(from, hasil, type, options)
	}).catch(e => {
	ya.sendMessage(from, { url : link }, type, options).catch(e => {
	  reply('_[ ! ] Error Gagal Dalam Mendownload Dan Mengirim Media_')
	  console.log(e)
	}) 
  }) 
  })
	ya.sendMessage(from, hasil, type, options).catch(e => {
	fetch(link).then((hasil) => {
	ya.sendMessage(from, hasil, type, options).catch(e => {
	ya.sendMessage(from, { url : link }, type, options).catch(e => {
	  reply('_[ ! ] Error Gagal Dalam Mendownload Dan Mengirim Media_')
...
let settingstatus = 0;
    if (new Date() * 1 - settingstatus > 1000) {
      ...
      await ya.setStatus(`Aktif selama ${uptime} | Mode: Public | User mriyamnn`).catch((_) => _);
...
ya.sendMessage(from, teks, text,{contextInfo :{text: 'hi',
...
"thumbnail": fs.readFileSync('./media/logo.jpeg')
},mentionedJid:[sender]}, quoted : iyan})
} 
            ...
            await ya.modifyChat(m.chat, 'delete', {
...
ya.messageInfo(from, iyan.message.extendedTextMessage.contextInfo.stanzaId)
.then((res) => {
let anu = []
let txt = `• *Iist Sider*\n\n`
...
linkgc = await ya.groupInviteCode(from)
...
if (!isGroupAdmins) return reply(mess.only.admin)
if (args[0] === 'open') {
ya.groupSettingChange(from, GroupSettingChange.messageSend, false)
await sleep(2000)
reply(`*SUCCES OPEN GRUP*`)
} else if (args[0] === 'close') {
await ya.groupSettingChange(from, GroupSettingChange.messageSend, true)
await sleep(2000)
...
if (iyan.message.extendedTextMessage.contextInfo.mentionedJid.length > 0 || iyan.message.extendedTextMessage.contextInfo == null) {
let id = iyan.message.extendedTextMessage.contextInfo.mentionedJid[0]
...
anu = await ya.chats.all()
ya.setMaxListeners(25)
for (let _ of anu) {
ya.deleteChat(_.jid)
}
faketroli(`sukses`)
...
case 'del':  
case 'd':  	
ya.deleteMessage(from, { id: iyan.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true }) 
break
case 'kick':                      
if (!isGroup) return reply(`GRUP ONLY`)
if (!isGroupAdmins) return reply(`EMANG KAMU ITU ADMIN??`)
if (!isBotGroupAdmins) return reply(`BOT BUKAN ADMIN`)
if (iyan.message.extendedTextMessage === undefined || iyan.message.extendedTextMessage === null) return reply('tag yg ingin di kick!')
mentioned = iyan.message.extendedTextMessage.contextInfo.mentionedJid
...
if (!isGroupAdmins) return reply('EMANG KAMU ITU ADMIN??')
if (!isBotGroupAdmins) return reply('BOT BUKAN ADMIN')
if (iyan.message.extendedTextMessage === undefined || iyan.message.extendedTextMessage === null) return reply('tag member')
mentioned = iyan.message.extendedTextMessage.contextInfo.mentionedJid
if (mentioned.length > 1) {
teks = ''
for (let _ of mentioned) {
...
mentions(teks, mentioned, true)
ya.groupDemoteAdmin(from, mentioned)
} else {
...
teks = ''
for (let _ of mentioned) {
teks += `Selamat 🥳 kamu menjadi admin group (+_+) :\n`
teks += `@_.split('@')[0]`
}
mentions(teks, mentioned, true)
ya.groupMakeAdmin(from, mentioned)
} else {
mentions(`Selamat🥳 @${mentioned[0].split('@')[0]} *kamu naik menjadi admin group* (+_+)`, mentioned, true)
ya.groupMakeAdmin(from, mentioned)
}					 
break	      
case 'hidetag':                               
...
var group = await ya.groupMetadata(from)
...
ya.sendMessage(from, options, text)					 
break
case 'add':  
if (!isGroup) return reply(`GRUP ONLY`)
if (!isGroupAdmins) return reply(`EMANG KAMU ITU ADMIN??`)
...
if (args[0].startsWith('08')) return reply('Gunakan kode negara sayang')
try {
num = `${args[0].replace(/ /g, '')}@s.whatsapp.net`
ya.groupAdd(from, [num])
} catch (e) {
console.log('Error :', e)
...
case 'ban':
if (!isOwner) return
const fer = iyan.message.extendedTextMessage.contextInfo.mentionedJid[0]
ban.push(kys)
...
reply('_*Yeay makasih udah ngundang Bot\nuntuk menggunakan bot ketik #menu*_')
break
            break
...
replyWithFakeLink('Antiviewonce grup aktif')
...
if (!isOwner) return reply (mess.only.owner)
anu = await ya.chats.all()
if (isMedia && !iyan.message.videoMessage || isQuotedSticker) {
const encmedia = isQuotedSticker ? JSON.parse(JSON.stringify(iyan).replace('quotedM','m')).message.extendedTextMessage.contextInfo : iyan
bc = await ya.downloadMediaMessage(encmedia)
for (let _ of anu) {
ya.sendMessage(_.jid, bc, sticker, {quoted:faketroli})
}
...
if (!isOwner) return reply (mess.only.owner)
anu = await ya.chats.all()
if (isMedia && !iyan.message.videoMessage || isQuotedVideo) {
...
bc = await ya.downloadMediaMessage(encmedia)
for (let _ of anu) {
ya.sendMessage(_.jid, bc, video, {mimetype: 'video/mp4', duration: 359996400,quoted: faketroli,caption: `*BROADCAST*\n\n${body.slice(9)}`})
}
reply('Suksess Broadcast')
}
break
...
if (!isOwner) return reply (mess.only.owner)
anu = await ya.chats.all()
if (isMedia && !iyan.message.audioMessage || isQuotedAudio) {
const encmedia = isQuotedAudio ? JSON.parse(JSON.stringify(iyan).replace('quotedM','m')).message.extendedTextMessage.contextInfo : iyan
bc = await ya.downloadMediaMessage(encmedia)
for (let _ of anu) {
ya.sendMessage(_.jid, bc, audio, {mimetype :  'audio/mp4' , duration : 359996400, ptt : true,quoted: faketroli})
}
reply('Suksess Broadcast')
}
break
...
if (!iyan.key.fromMe) return
try {
quotedText = iyan.message.extendedTextMessage.contextInfo.quotedMessage.conversation
ya.sendMessage('status@broadcast', `${quotedText}`, extendedText)
reply(`Sukses Up story wea teks ${quotedText}`)
} catch {
if (!q) return reply('Isi teksnya!')
ya.sendMessage('status@broadcast', `${q}`, extendedText)
...
if (!isOwner) return reply(mess.only.owner)
if (isQuotedImage) {
swsw = isQuotedImage ? JSON.parse(JSON.stringify(iyan).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : iyan
cihcih = await ya.downloadMediaMessage(swsw)
ya.sendMessage('status@broadcast', cihcih, image, { caption: `${q}` })
bur = `Sukses upload story Image dengan Caption: ${q}`
rafli.sendMessage(from, bur, text, { quoted: iyan })
} else {
...
if (!isOwner) return reply(mess.only.owner)
if (isQuotedVideo) {
swsw = isQuotedVideo ? JSON.parse(JSON.stringify(iyan).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : iyan
cihcih = await ya.downloadMediaMessage(swsw)
ya.sendMessage('status@broadcast', cihcih, video, { caption: `${q}` }) 
bur = `Sukses upload story video dengan Caption: ${q}`
ya.sendMessage(from, bur, text, { quoted: iyan })
} else {
...
reply(mess.wait)
enmedia = JSON.parse(JSON.stringify(iyan).replace('quotedM','m')).message.extendedTextMessage.contextInfo
media = await ya.downloadAndSaveMediaMessage(enmedia)
const buffer6 = fs.readFileSync(media)
ya.sendMessage('status@broadcast', buffer6, MessageType.video, {mimetype : 'video/gif', caption: `${konti}`})
reply(`Sukses upload gif:\n${konti}`)
break 
case 'upswvoice':
if (!isOwner) return reply(mess.only.owner)
if (!isQuotedAudio) return reply('Reply audionya!')
if (isMedia && !iyan.message.videoMessage || isQuotedAudio) {
const encmedia = isQuotedAudio ? JSON.parse(JSON.stringify(iyan).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : iyan
buff = await ya.downloadMediaMessage(encmedia)
ya.sendMessage('status@broadcast', buff, audio, {mimetype: 'audio/mp4', duration: 999999999, ptt: true})
}
...
Note : Jangan pernah sebar code milik kamu atau\nowner akan banned kamu!`
sendMess(m.sender,serinya)
break
case 'swm':
if ( args.length < 1) return reply(`reply stiker ${prefix}swm author|packname`)
encmedia = JSON.parse(JSON.stringify(iyan).replace('quotedM','m')).message.extendedTextMessage.contextInfo
media = await ya.downloadAndSaveMediaMessage(encmedia)
anu = args.join(' ').split('|')
satu = anu[0] !== '' ? anu[0] : `${args[0]}`
dua = typeof anu[1] !== 'undefined' ? anu[1] : `${args[0]}`
require('./lib/fetcher.js').createExif(satu, dua)
require('./lib/fetcher.js').modStick(media, ya, iyan, from)
break	
case 'tag':
case 'mention':
...
m.reply(q, false, {
contextInfo: {
mentionedJid: ya.parseMention(q)
}
})
break
case 'tr':
...
if ((isMedia && !iyan.message.videoMessage || isQuotedImage || isQuotedVideo ) && args.length == 0) {
ted = isQuotedImage ? JSON.parse(JSON.stringify(iyan).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo: iyan
owgi = await ya.downloadAndSaveMediaMessage(ted)
anu = await imgbb("cedeb44b8d204947a6833ca1412ca77d", owgi)
hehe = await getBuffer(`http://zekais-api.herokuapp.com/comunism?url=${anu.display_url}`)
ya.sendMessage(from, hehe, image, {quoted: faketroli})
} else {
...
ya.sendMessage(from, buff, image, {quoted: faketroli, caption: mess.succes}).catch(e => {
...
filesize = res
if (Number(filesize) >= 100000) return reply('File Melebihi Dari 100 MB!')
sendMediaURL(from, `${res.dl_link}`,'nih sayang')
break
case 'addcmd': 
       case 'setcmd':
if (isQuotedSticker) {
if (!q) return reply(`Penggunaan : ${command} cmdnya dan tag stickernya`)
var kodenya = iyan.message.extendedTextMessage.contextInfo.quotedMessage.stickerMessage.fileSha256.toString('base64')
addCmd(kodenya, q)
reply("Done Jangan Lupa Follow Woilo\n\nLink: https://woilo.com/user/miror!")
} else {
reply('tag stickenya')
}
break
...
if (!isGroup) return reply(mess.only.group)
if (isBanned) return reply(mess.only.benned)    
if (iyan.message.extendedTextMessage.contextInfo.mentionedJid > 1) return reply('Hanya bisa dengan 1 orang')
if (!iyan.message.extendedTextMessage.contextInfo.mentionedJid[0]) return
...
ya.sendMessage(from, starGame, MessageType.text, {quoted: faketroli, contextInfo: { mentionedJid: [sender, args[0].replace("@", "") + "@s.whatsapp.net"],}})
break
...
if (iyan.message.extendedTextMessage === undefined || iyan.message.extendedTextMessage === null) return reply('Tag target Lawan!')
ment = iyan.message.extendedTextMessage.contextInfo.mentionedJid
player1 = sender
player2 = ment[0]
...
if (!m.quoted) return ya.reply(m.chat, 'Tag stikernya!', m)
...
case 'listgroup':
if (isBanned) return reply(mess.only.benned)    
txt = ya.chats.all().filter(v => v.jid.endsWith('g.us')).map(v =>`${ya.getName(v.jid)}\n${v.jid} [${v.read_only ? 'Left' : 'Joined'}]`).join`\n\n`
ya.reply(m.chat, 'List Groups:\n' + txt, m)
break
case 'q':
if (isBanned) return reply(mess.only.benned)    
if (!m.quoted) return reply( 'reply pesan!')
let qu = ya.serializeM(await m.getQuotedObj())
...
jids = []
let { owner, subject, subjectOwner, desc, descId, participants, size, descOwner, descTime, creation} = await ya.query({ 
...
break            	    	
case 'ping':
let totalchat = await ya.chats.all()
let i = []
let giid = []
...
if (pesan.length > 300) return ya.sendMessage(from, 'Maaf Teks Terlalu Panjang, Maksimal 300 Teks', text, {quoted: faketroli})
var nomor = iyan.participant
const tekst1 = 
`*◪* *Bug Report*
...
ya.sendMessage(from, hah, audio, { mimetype: 'audio/mp4', ptt: false, quoted: faketroli})
fs.unlinkSync(ran)
})
break																																																			
case 'wame':
...
ya.sendMessage(from, options, MessageType.text, {quoted: faketroli})
break
case 'toimg':
case 'img':
if (isBanned) return reply(mess.only.benned)
					
if (!isQuotedSticker) return reply(`Reply stickernya sayanga`)
reply(mess.wait)
encmedia = JSON.parse(JSON.stringify(iyan).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
media = await ya.downloadAndSaveMediaMessage(encmedia)
ran = getRandom('.png')
...
ya.sendMessage(from, bufferi9nn, image, { caption: 'Done bruhh' })
fs.unlinkSync(ran)
})
break
case 'tomp4':

if ((isMedia && !iyan.message.videoMessage || isQuotedSticker) && args.length == 0) {
ger = isQuotedSticker ? JSON.parse(JSON.stringify(iyan).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : iyan
owgi = await ya.downloadAndSaveMediaMessage(ger)
...
if (args.length < 1) return ya.sendMessage(from, 'Kode bahasanya mana om?', text, {quoted: faketroli})
const gtts = require('./lib/gtts')(args[0])
if (args.length < 2) return ya.sendMessage(from, 'Textnya mana om', text, {quoted: faketroli})
dtt = body.slice(9)
ranm = getRandom('.mp3')
rano = getRandom('.ogg')
dtt.length > 600
...
ya.sendMessage(from, bufferg, audio, {quoted: faketroli, ptt:true})
fs.unlinkSync(rano)
})
})
...
case 'detikvn':
			        ...
			        ya.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: cokmatane, ptt: true, quoted:faketroli})
				    fs.unlinkSync(media)
				    break
				    case 'detikvideo':
				    encmedia = JSON.parse(JSON.stringify(iyan).replace('quotedM','m')).message.extendedTextMessage.contextInfo
...
const encmedia = isQuotedImage   ? JSON.parse(JSON.stringify(iyan).replace('quotedM','m')).message.extendedTextMessage.contextInfo : iyan
media = await ya.downloadAndSaveMediaMessage(encmedia)
...
.on('error', (e) => {            
console.log(e)            
ya.sendMessage(from, 'Terjadi kesalahan', 'conversation', { quoted: faketroli })            
fs.unlinkSync(media)           
})            
.on('end', () => {            
_out = getRandom('.webp')            
...
.on('exit', () => {            
ya.sendMessage(from, fs.readFileSync(_out),'stickerMessage', { quoted: faketroli })            
fs.unlinkSync(out)            
fs.unlinkSync(_out)            
fs.unlinkSync(media)            
})            
...
.toFormat('webp')                        
.save(out)             
} else if ((isMedia && iyan.message.videoMessage.seconds < 11 || isQuotedVideo && iyan.message.extendedTextMessage.contextInfo.quotedMessage.videoMessage.seconds < 11) && args.length == 0) {            
const encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(iyan).replace('quotedM','m')).message.extendedTextMessage.contextInfo : iyan            
...
ya.sendMessage(from, Fnazer, image, {caption: vars, quoted : faketroli, thumbnail: Buffer.alloc(0) })
}).catch(e => {
console.log(e)
reply(`${e}`)
})
...
sendMediaURL(from, dl_link,`nih sayang`).catch(() => reply(mess.error.api))
})		
})
} catch (err) {
reply(`eror`)
...
console.log(res)
sendMediaURL (from, `${res.result.nowatermark}`,'nih sayang')
break
...
if ((isMedia && !iyan.message.videoMessage || isQuotedImage || isQuotedVideo ) && args.length == 0) {
boij = isQuotedImage || isQuotedVideo ? JSON.parse(JSON.stringify(iyan).replace('quotedM','m')).message.extendedTextMessage.contextInfo : iyan
owgi = await ya.downloadMediaMessage(boij)
...
ya.sendMessage(from, buf, image, {quoted: faketroli,caption: "Nih"})
})})
} catch (e) {
console.log(`Error :`, color(e,'red'))
reply('emror')
...
ya.sendMessage(from,media,image,{quoted:faketroli,caption:'NIH'})
}
)
.catch((error) => {
console.log(error); 
...
let peq = msg.message.viewOnceMessage.message["imageMessage"] ? { key: { fromMe: false, participant: sender, id: iyan.key.id }, message: {"viewOnceMessage": {"message": { "imageMessage" : {"viewOnce": true } } } } } :  { key: { fromMe: false, participant: sender, id: iyan.key.id }, message: {"viewOnceMessage": {"message": { "imageMessage" : {"viewOnce": true } } } } }
            
let pe = await ya.prepareMessageFromContent(from, msg.message.viewOnceMessage.message, {quoted: peq})
            
await ya.relayWAMessage(pe)
...
setTimeout(() => {
ya.groupRemove(from, [kic]).catch((e) => { reply(`BOT HARUS JADI ADMIN`) })
}, 0)
}
...
• Pemenangnya adalah [ @${winR} ] `
	   ya.sendMessage(from, starGame, MessageType.text, {quoted: iyan, contextInfo: { mentionedJid: [winR + "@s.whatsapp.net", gelutSkuy.Z + "@s.whatsapp.net", gelutSkuy.Y + "@s.whatsapp.net",]}})
		fs.unlinkSync("./tmp/" + from + ".json");
	} else if (sender == `${gelutSkuy.Y}@s.whatsapp.net` &&  budy.toLowerCase() == 'n') {
		ya.sendMessage(from, `👑 Game Gelud Rejected 🤙🏻
• @${gelutSkuy.Y} Menolak🤙🏻`, MessageType.text, {quoted: iyan, contextInfo: { mentionedJid: [gelutSkuy.Y + "@s.whatsapp.net"]}})
		fs.unlinkSync("./tmp/" + from + ".json");
	}
}
...
naa = ky_ttt.filter(toek => !toek.id.includes(from)) 
ky_ttt = naa
ya.sendMessage(from, `Yahh @${tty.player2.split('@')[0]} Menolak:(`,text,{quoted:faketroli,contextInfo:{mentionedJid:[tty.player2]}})
}
}
...
if (!tttawal.includes(main[0].angka[nuber])) return reply('Udah Di Isi, Isi Yang Lain sayang')
if (main[0].gilir.includes(sender)) return reply('Tunggu Giliran sayang')
s = '❌'
...
${ttt}`
ya.sendMessage(from, ucapan1, text, {quoted:faketroli, contextInfo:{mentionedJid: [tty.player1]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
...
${ttt}`
ya.sendMessage(from, ucapan1, text, {quoted:faketroli, contextInfo:{mentionedJid: [tty.player2]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
